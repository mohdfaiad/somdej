...
type
  TfrmRrReceiveMoneyGrid = class(TMainGridForm)
    dsgrid: TDataSource;
    UniLabel3: TUniLabel;
    EdtCom: TUniEdit;
    UniLabel2: TUniLabel;
...
var
  frmRrReceiveMoneyGrid: TfrmRrReceiveMoneyGrid;

implementation

{$R *.dfm}

uses uniGUIForm, XeUniGUIComponents, RrReceiveMoneyDataForm;

procedure TfrmRrReceiveMoneyGrid.ShowData();
var
  _Form: TUniForm;
begin
{  if FG = 'R' then // ทะเบียนป้ายแดง
  ...
  _Form := TUniFormClass(TfrmRrReceiveMoneyDataT).Create(UniApplication);
  TfrmRrReceiveMoneyDataT(_Form).Init(FDM,FG);
...
procedure TfrmRrReceiveMoneyGrid.btnAddClick(Sender: TObject);
...
procedure TfrmRrReceiveMoneyGrid._Confirm(AResult: Integer);
begin
  case AResult of
    mrYes:
      begin
...
procedure TfrmRrReceiveMoneyGrid.btnDeleteClick(Sender: TObject);
begin
...
procedure TfrmRrReceiveMoneyGrid.btnEditClick(Sender: TObject);
begin
   if (not Fdm.CdsTtRcvGet.Active) or
    (Fdm.CdsTtRcvGet.IsEmpty) then
  begin
...
procedure TfrmRrReceiveMoneyGrid.btnSearchClick(Sender: TObject);
var
    cirt : String;
begin
...
procedure TfrmRrReceiveMoneyGrid.BtnViewClick(Sender: TObject);
begin
   if (not Fdm.CdsTtRcvGet.Active) or
    (Fdm.CdsTtRcvGet.IsEmpty) then
  begin
...
procedure TfrmRrReceiveMoneyGrid.dbgDataListTitleClick(Column: TUniDBGridColumn);
begin
...
procedure TfrmRrReceiveMoneyGrid.UniFormCreate(Sender: TObject);
begin
...
procedure TfrmRrReceiveMoneyGrid.UniFormDestroy(Sender: TObject);
begin
  inherited;
