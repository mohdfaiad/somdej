...
type
  TfrmRrReceiveMoneyDataT = class(TMainDataForm)
    LbDnort4: TUniLabel;
    PnHeadRt4: TUniPanel;
    Lbidtrt4: TUniLabel;
    PnDtl1: TUniPanel;
    ...
    procedure BtnSelectCSMorVMClick(Sender: TObject);
    procedure PTPRT4CloseUp(Sender: TObject);
...
var
  frmRrReceiveMoneyDataT: TfrmRrReceiveMoneyDataT;
...
uses PopupCsmForm, PopupArForm, Math,
   PopupAddressForm, PopupRcvMoneyForm, PopupRedRegRcvAm4Form;


Procedure TfrmRrReceiveMoneyDataT.PrintReport(_Sql_Txt : String);
var
...
procedure TfrmRrReceiveMoneyDataT.PTPRT4CloseUp(Sender: TObject);
begin
  PnDtl3.Visible := (Fdm.CdsTtRcvGetPTPRT4.AsString  = 'P');

end;

procedure TfrmRrReceiveMoneyDataT.ShowEdtAmtAndDate();
begin
...
procedure TfrmRrReceiveMoneyDataT.GetAm4;
begin
    // Receive Red Card FG = R  only becouse this form use red card .
    FDCD := '102';

  Fdm.CdsTtRrRcvGetAm4.Close;
  if Fdm.TtRcvGetAm4(Fdm.CdsTtRcvGetYARRT4.AsInteger,Fdm.CdsTtRcvGetCMPRT4.AsString,
     Fdm.CdsTtRcvGetBRNRT4.AsString,FDCD,Fdm.CdsTtRcvGetCTYRT4.AsString,
     Fdm.CdsTtRcvGetCNORT4.AsLargeInt,FG) then
    Fdm.CdsTtRrRcvGetAm4.Open
  else
    ShowMessage('เกิดข้อผิดพลาดเข้าถึงข้อมูล');
end;

procedure TfrmRrReceiveMoneyDataT.GetRt4;
begin
...
procedure TfrmRrReceiveMoneyDataT.GetGt1;
begin
...
procedure TfrmRrReceiveMoneyDataT.CustomerSelected(CCNCSM:int64;TNMCSM:String);
begin
...
procedure TfrmRrReceiveMoneyDataT.btnCancelClick(Sender: TObject);
begin
  if Fdm.CdsTtRcvGet.State in [dsInsert, dsEdit] then
    Fdm.CdsTtRcvGet.Cancel;

  if Fdm.CdsTtRrRcvGetAm4.Active then
     Fdm.CdsTtRrRcvGetAm4.Close;
  ...
  if FSetupdm.CdsStVm1Get.Active then
     FSetupdm.CdsStVm1Get.Close;
...
procedure TfrmRrReceiveMoneyDataT.btnConfirmClick(Sender: TObject);
...
procedure TfrmRrReceiveMoneyDataT.checkCur(Sender: TObject;CdsTmp : TClientDataSet);
var
  cur : Currency;
  F: TField;
  Str,Tmp : String;
...
procedure TfrmRrReceiveMoneyDataT.AMTRT4Exit(Sender: TObject);
begin
  checkCur(AMTRT4,fdm.CdsTtRcvGet);

end;

procedure TfrmRrReceiveMoneyDataT.AMTRT4KeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
  begin
    key := #0;
...
procedure TfrmRrReceiveMoneyDataT.CallSumAmtRt4();
var SumAmt : Double;
begin
  SumAmt := 0;
  ...
  begin
   ...
   AMTRT4.Text := EdtSumAmtRt4.Text;
   checkCur(AMTRT4,fdm.CdsTtRcvGet);
end;

procedure TfrmRrReceiveMoneyDataT._SumAmtRt4(SumAmtRt4 : Double);
begin
...
procedure TfrmRrReceiveMoneyDataT.BtnAddRt4Click(Sender: TObject);
begin

    with TfrmPopupRedRegRcvAm4.Create(UniApplication) do
    begin
      Init(FDM,PTPRT4.Text,BAKRT4.Text,FG);

      OnRedSumAmtRt4 := _SumAmtRt4;
      ShowModal();
    end;

end;

procedure TfrmRrReceiveMoneyDataT._Confirm(AResult: Integer);
begin
  case AResult of
...
procedure TfrmRrReceiveMoneyDataT.BtnDeleteRt4Click(Sender: TObject);
begin
...
procedure TfrmRrReceiveMoneyDataT.BtnPrintClick(Sender: TObject);
var Str_Sql : String;
begin
   Str_Sql :='SELECT * FROM SP_REP_RM_RCV_MONEY(' +
              Fdm.CdsTtRcvGetYARRT4.AsString + ',' +
...
procedure TfrmRrReceiveMoneyDataT.GetAddr(cno : int64);
begin
...
procedure TfrmRrReceiveMoneyDataT.GetAddrAr(cno : String);
begin
  FCarSaledm.CdsMtCsmGetAddr01.Close();
...
procedure TfrmRrReceiveMoneyDataT.ArSelected(CNOCM1,TNMCM1,CTPCM1,SMNCM1 : String;CRTAM3 : INTEGER);
begin
  Fdm.CdsTtRcvGetCNORT4.AsLargeInt := StrToInt(CNOCM1);
  Fdm.CdsTtRcvGetTNMCSM.AsString := TNMCM1;

  GetAddrAr(fdm.CdsTtRcvGetCNORT4.AsString);
  GetAm4

end;

procedure TfrmRrReceiveMoneyDataT.BtnSelectCSMorVMClick(Sender: TObject);
begin

  if fdm.CdsTtRcvGetCTYRT4.AsString = '2' then
  begin
...
procedure TfrmRrReceiveMoneyDataT.Init(DM : TDSReceiveMoneyDM; _FG : STRING);
begin
 ...
 //   ShowEdtAmtAndDate();
  ...
  begin
     ...
     BtnSelectCSMorVM.Enabled := False;
