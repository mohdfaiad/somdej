...
begin
  checkCur(FPAOT6, FDM.CdsMtAppGetAccessory);
...
begin
  ...
  FU1OT6.Text := FormatCurr('#,##0.00', Fdm.CdsMtAppGetAccessoryFU1OT6.AsFloat);
  FU2OT6.Text := FormatCurr('#,##0.00', Fdm.CdsMtAppGetAccessoryFU2OT6.AsFloat);

  FQ1OT6.Text := FormatCurr('#,##0.00', Fdm.CdsMtAppGetAccessoryFQ1OT6.AsFloat);
  FQ2OT6.Text := FormatCurr('#,##0.00', Fdm.CdsMtAppGetAccessoryFQ2OT6.AsFloat);

  AM1OT6.Text := FormatCurr('#,##0.00', Fdm.CdsMtAppGetAccessoryAM1OT6.AsFloat);
  AM2OT6.Text := FormatCurr('#,##0.00', Fdm.CdsMtAppGetAccessoryAM2OT6.AsFloat);

  AMTOT6.Text := FormatCurr('#,##0.00', Fdm.CdsMtAppGetAccessoryAMTOT6.AsFloat);
  FPAOT6.Text := FormatCurr('#,##0.00', Fdm.CdsMtAppGetAccessoryFPAOT6.AsFloat);
  NETOT6.Text := FormatCurr('#,##0.00', Fdm.CdsMtAppGetAccessoryNETOT6.AsFloat);

end;

procedure TfrmPopupAccessory.TDSOT6CloseUp(Sender: TObject);
begin
    FDM.CdsMtAppGetAccessoryFP1OT6.AsFloat := 0.00;
    FDM.CdsMtAppGetAccessoryFP2OT6.AsFloat := 0.00;
    FDM.CdsMtAppGetAccessoryFP3OT6.AsFloat := 0.00;
    FDM.CdsMtAppGetAccessoryFP4OT6.AsFloat := 0.00;

    if (FDM.CdsMtAppGetAccessoryTDSOT6.AsString = '1') then
      FDM.CdsMtAppGetAccessoryFP1OT6.AsFloat := FDM.CdsMtAppGetAccessoryFPAOT6.AsFloat
    else if (FDM.CdsMtAppGetAccessoryTDSOT6.AsString = '2') then
      FDM.CdsMtAppGetAccessoryFP2OT6.AsFloat := FDM.CdsMtAppGetAccessoryFPAOT6.AsFloat
    else if (FDM.CdsMtAppGetAccessoryTDSOT6.AsString = '3') then
      FDM.CdsMtAppGetAccessoryFP3OT6.AsFloat := FDM.CdsMtAppGetAccessoryFPAOT6.AsFloat
    else if (FDM.CdsMtAppGetAccessoryTDSOT6.AsString = '4') then
      FDM.CdsMtAppGetAccessoryFP4OT6.AsFloat := FDM.CdsMtAppGetAccessoryFPAOT6.AsFloat;
end;
...
begin
  ...
  ch := False;
  if fdm.CdsMtAppGetAccessory.State in [dsInsert] then
  begin
    cnt := _StrList.Count - 1;
    for I := 0 to cnt do
    begin
      if _StrList[i] = fdm.CdsMtAppGetAccessoryFSROT6.AsString then
      begin
        ch := True;
        Break;
      end;
  ...
  begin
    fdm.CdsMtAppGetAccessory.Post();
    Close;
  end;

//  if ch then
//  begin
//    CNOCDE :=  Fdm.CdsMtAppGetAccessoryCNOCDE.AsString;
//    ShowMessage('บันทึกเรียบร้อยแล้ว');
//    FDM.CdsMtAppGetAccessory.Close;
//    FDM.CdsMtAppGetAccessory.Open;
//    Fdm.CdsMtAppGetAccessory.Locate('CNOCDE',CNOCDE,[]);
...
begin
  Fdm.CdsMtAppGetAccessoryFSROT6.AsString := ITNPM1;
  Fdm.CdsMtAppGetAccessoryTNMPM1.AsString := TNMPM1;

  Fdm.CdsMtAppGetAccessoryFU1OT6.AsFloat := UP1PM1;
  Fdm.CdsMtAppGetAccessoryFU2OT6.AsFloat := UP2PM1;
...
begin
   ...
   dsgrid.DataSet := Fdm.CdsMtAppGetAccessory;
  ...
  else if (fdm.CdsMtAppGetAccessory.State in [dsEdit]) then
  begin
