...
type
  TfrmCsAppSaleCarData = class(TMainDataForm)
    ...
    procedure BtnAccessoryViewClick(Sender: TObject);
    procedure BtnAccessoryEditClick(Sender: TObject);
    procedure BtnAccessoryDelClick(Sender: TObject);
    procedure BtnExpDelClick(Sender: TObject);
    procedure CTYOT5CloseUp(Sender: TObject);
  ...
  private
    ...
    procedure ArSelected(CNOCM1,TNMCM1,CTPCM1,SMNCM1 : String;CRTAM3 : INTEGER);
    procedure BrokerSelected(CNOvm1,TNMvm1,CTPvm1,psNvm1: String;CRTYM3 : INTEGER;cspvm1 :String);
    procedure InsuranceSelected(CNOCM1,TNMCM1,CTPCM1,SMNCM1 : String;CRTAM3 : INTEGER);
    procedure FinanceSelected(CNOCM1,TNMCM1,CTPCM1,SMNCM1 : String;CRTAM3 : INTEGER);
    procedure GnoSelected(CCNCSM:int64;TNMCSM:String);
    procedure _ConfirmAccessory(AResult: Integer);
    procedure _ConfirmEXP(AResult: Integer);
  ...
  public
     ...
     procedure GetAddrAr(cno : String);
...
uses PopupProductForm, PopupCsmForm, PopupApForm, PopupProductIM1Form,PopupRCVCardForm, Math,
  PopupExpForm, PopupAccessoryForm, PopupDownForm, PopupArForm;
...
begin
  ...
  if fdm.CdsMtAppGetAccessoryMs.Active then
  begin
     fdm.CdsMtAppGetAccessoryMs.First();
    while not fdm.CdsMtAppGetAccessoryMs.Eof do
    begin
        chAcc:= FDM.MtAppInsUpdAccessory(
            Fdm.CdsMtAppGetAccessoryMsSTSOT6.Asstring  ,
            Fdm.CdsMtAppGetAccessoryMsYAROT6.AsInteger ,
            Fdm.CdsMtAppGetAccessoryMsCMPOT6.Asstring ,
            Fdm.CdsMtAppGetAccessoryMsBRNOT6.Asstring  ,
            Fdm.CdsMtAppGetAccessoryMsDCDOT6.Asstring ,
            OUT_DNO,
            Fdm.CdsMtAppGetAccessoryMsSEQOT6.AsInteger ,
            Fdm.CdsMtAppGetAccessoryMsFGDOT6.Asstring  ,
            Fdm.CdsMtAppGetAccessoryMsFRDOT6.Asstring,
            Fdm.CdsMtAppGetAccessoryMsFSROT6.Asstring  ,
            Fdm.CdsMtAppGetAccessoryMsFWHOT6.Asstring,
            Fdm.CdsMtAppGetAccessoryMsFPDOT6.Asstring ,
            Fdm.CdsMtAppGetAccessoryMsFCLOT6.Asstring,
            Fdm.CdsMtAppGetAccessoryMsFQ1OT6.AsInteger,
            Fdm.CdsMtAppGetAccessoryMsFQ2OT6.AsInteger ,
            Fdm.CdsMtAppGetAccessoryMsTDSOT6.Asstring,
            Fdm.CdsMtAppGetAccessoryMsFDSOT6.AsFloat ,
            Fdm.CdsMtAppGetAccessoryMsFPAOT6.AsFloat  ,
            Fdm.CdsMtAppGetAccessoryMsFU1OT6.AsFloat  ,
            Fdm.CdsMtAppGetAccessoryMsFU2OT6.AsFloat  ,
            Fdm.CdsMtAppGetAccessoryMsPRTOT6.Asstring  ,
            Fdm.CdsMtAppGetAccessoryMsABBOT6.Asstring ,
            Fdm.UserID
          );

      fdm.CdsMtAppGetAccessoryMs.next;
    end;
  end;
...
begin
   ...
   RDTOT5.Text := FDM.FSBuddhistDateToStr(fDM.CdsMtAppGetRDTOT5.AsDateTime);

  // STAOT5.Text := FDM.FSBuddhistDateToStr(fDM.CdsMtAppGetSTAOT5.AsDateTime);
  // ENDOT5.Text := FDM.FSBuddhistDateToStr(fDM.CdsMtAppGetENDOT5.AsDateTime);
...
var Finance , Cash : Boolean;
begin
   Finance := (Fdm.CdsMtAppGetSTPOT5.AsString = 'F');
   Cash := (Fdm.CdsMtAppGetSTPOT5.AsString = 'S');
   ...
   DWTOT5.Visible := NOT Cash;
   NLEOT5.Visible := NOT Cash;
   RATOT5.Visible := NOT Cash;
   PMMOT5.Visible := NOT Cash;
   CM1OT5.Visible := NOT Cash;
   SUMOT5.Visible := NOT Cash;
   HPOT5.Visible := NOT Cash;
   CM2OT5.Visible := NOT Cash;
   ESDOT5.Visible := NOT Cash;
   DDTOT5.Visible := NOT Cash;

   LbFirst.Visible := NOT Cash;
   LbDueType.Visible := NOT Cash;
   LbDown.Visible := NOT Cash;
   LbPeriod.Visible := NOT Cash;
   Lbdown2.Visible := NOT Cash;
   LbRate.Visible := NOT Cash;
   LbInstallAmt.Visible := NOT Cash;
   LbHpAmt.Visible := NOT Cash;
   LbAmt.Visible := NOT Cash;
   LbFeeDown.Visible := NOT Cash;
...
begin
  FSetupdm.CdsSTCM1GetAddr01.Close();
  TA1CSM.DataField := 'TA1CSM';
  TA2CSM.DataField := 'TA2CSM';
  CHGCSM.DataField := 'CHGCSM';
  TELCSM.DataField := 'TELCSM';
...
procedure TfrmCsAppSaleCarData.GetAddrAr(cno : String);
begin
  Fdm.CdsMtCsmGetAddr01.Close();
  TA1CSM.DataField := 'TA1CMA';
  TA2CSM.DataField := 'TA2CMA';
  CHGCSM.DataField := 'CHGCMA';
  TELCSM.DataField := 'TELCMA';
  dsAddr01.DataSet := FSetupdm.CdsSTCM1GetAddr01;
  FSetupdm.CdsSTCM1GetAddr01.Close();
  FSetupdm.StCM1GetAddr(cno,'1');
  FSetupdm.CdsSTCM1GetAddr01.Open();

  // เสร็จแล้วโยนค่าลงEdittext ให้ Edit text cmo dbedit addr
end;
...
procedure TfrmCsAppSaleCarData.CTYOT5CloseUp(Sender: TObject);
begin
  Fdm.CdsMtAppGetCNOOT5.AsInteger := 0;
  Fdm.CdsMtAppGetTNMCSM.AsString := '';
...
procedure TfrmCsAppSaleCarData.ArSelected(CNOCM1,TNMCM1,CTPCM1,SMNCM1 : String;CRTAM3 : INTEGER);
begin
  Fdm.CdsMtAppGetCNOOT5.AsLargeInt := StrToInt(CNOCM1);
  Fdm.CdsMtAppGetTNMCSM.AsString := TNMCM1;

  GetAddrAr(fdm.CdsMtAppGetCNOOT5.AsString);
  GetCard(fdm.CdsMtAppGetCNOOT5.AsLargeInt);
end;
...
begin
  ...
  begin
    ...
    Fdm.CdsMtAppGetFDTOT5.AsDateTime := IncMonth(Fdm.CdsMtAppGetSDTOT5.AsDateTime, 12*544);
    BDTOT5.Text := FDM.FSBuddhistDateToStr(fDM.CdsMtAppGetBDTOT5.AsDateTime);
    FDTOT5.Text := FDM.FSBuddhistDateToStr(fDM.CdsMtAppGetFDTOT5.AsDateTime);
  end;
...
begin
  ...
  while not fdm.CdsMtAppGetAccessoryMs.eof do
  begin
     StrListaAccessory.Add(fdm.CdsMtAppGetAccessoryMsFSROT6.AsString);
     fdm.CdsMtAppGetAccessoryMs.next;
  end;

  SeqNo := fdm.CdsMtAppGetAccessoryMs.RecordCount + 1;

  fdm.CdsMtAppGetAccessoryMs.Append();
  fdm.CdsMtAppGetAccessoryMsSTSOT6.AsString := 'A';
  fdm.CdsMtAppGetAccessoryMsSEQOT6.AsInteger := SeqNo;
  fdm.CdsMtAppGetAccessoryMsFQ1OT6.AsInteger := 1;
  fdm.CdsMtAppGetAccessoryMsFQ2OT6.AsInteger := 0;
  Fdm.CdsMtAppGetAccessoryMsSTSOT6.Asstring  := Fdm.CdsMtAppGetSTSOT5.Asstring;
  Fdm.CdsMtAppGetAccessoryMsYAROT6.AsInteger := Fdm.CdsMtAppGetYAROT5.AsInteger;
  Fdm.CdsMtAppGetAccessoryMsCMPOT6.Asstring  :=  Fdm.CdsMtAppGetCMPOT5.Asstring;
  Fdm.CdsMtAppGetAccessoryMsBRNOT6.Asstring  := Fdm.CdsMtAppGetBRNOT5.Asstring;
  Fdm.CdsMtAppGetAccessoryMsDCDOT6.Asstring  := Fdm.CdsMtAppGetDCDOT5.Asstring;
  Fdm.CdsMtAppGetAccessoryMsDNOOT6.AsInteger := Fdm.CdsMtAppGetDNOOT5.AsInteger;
  ShowAccessoryData();
...
procedure TfrmCsAppSaleCarData._ConfirmAccessory(AResult: Integer);
begin
  case AResult of
    mrYes:
      begin
          fdm.CdsMtAppGetAccessoryMs.Delete();
      end;
  end;
end;


procedure TfrmCsAppSaleCarData._ConfirmEXP(AResult: Integer);
begin
  case AResult of
    mrYes:
      begin
          fdm.CdsMtAppGetEXP.Delete();
      end;
  end;
end;

procedure TfrmCsAppSaleCarData.BtnAccessoryDelClick(Sender: TObject);
begin
   if (not Fdm.CdsMtAppGetAccessoryMs.Active) or
    (Fdm.CdsMtAppGetAccessoryMs.IsEmpty) then
  begin
    MessageDlg('There are no data selected.', mtConfirmation, [mbOK], nil);
    Exit;
  end;
...
procedure TfrmCsAppSaleCarData.BtnAccessoryEditClick(Sender: TObject);
begin
   if (not Fdm.CdsMtAppGetAccessoryMs.Active) or
    (Fdm.CdsMtAppGetAccessoryMs.IsEmpty) then
  begin
  ...
  Fdm.CdsMtAppGetAccessoryMs.Edit();
  ShowAccessoryData();

end;

procedure TfrmCsAppSaleCarData.BtnAccessoryViewClick(Sender: TObject);
begin
   if (not Fdm.CdsMtAppGetAccessoryMs.Active) or
    (Fdm.CdsMtAppGetAccessoryMs.IsEmpty) then
  begin
...
procedure TfrmCsAppSaleCarData.InsuranceSelected(CNOCM1,TNMCM1,CTPCM1,SMNCM1 : String;CRTAM3 : INTEGER);
begin
  if InsFlag = 1 then   // พ.ร.บ.
  begin
    Fdm.CdsMtAppGetICDOT5.AsString := CNOCM1;
    Fdm.CdsMtAppGetTNMICD.AsString := TNMCM1;
  end
  else  // ประกันชั้น 1
  begin
    Fdm.CdsMtAppGetISDOT5.AsString := CNOCM1;
    Fdm.CdsMtAppGetTNMISD.AsString := TNMCM1;
  end;
end;
...
procedure TfrmCsAppSaleCarData.FinanceSelected(CNOCM1,TNMCM1,CTPCM1,SMNCM1 : String;CRTAM3 : INTEGER);
begin
    Fdm.CdsMtAppGetFCDOT5.AsString := CNOCM1;
    Fdm.CdsMtAppGetTNMFCD.AsString := TNMCM1;
end;
...
procedure TfrmCsAppSaleCarData.BtnExpDelClick(Sender: TObject);
begin
...
begin
  ...
  if Fdm.CdsMtAppGetCTYOT5.AsString = '2' then // personal
    with TfrmPopupCsm.Create(UniApplication) do
    begin
    ...
    with TfrmPopupAr.Create(UniApplication) do    // juristic
    begin
      Init(FSetupdm, 'เลือกนิติบุคคล', 'A','ALL','');

      OnSelectData := ArSelected;
      ShowModal();
    end;
...
begin
    with TfrmPopupAr.Create(UniApplication) do    // juristic
    begin
      Init(FSetupdm, 'เลือกบริษัทไฟแนนซ์', 'A','ALL','12');  // 12 IS FINANCE COMPANY
...
begin
  InsFlag := 1;
    with TfrmPopupAr.Create(UniApplication) do    // juristic
    begin
      Init(FSetupdm, 'เลือกบริษัทประกัน', 'A','ALL','61');  // ุๅ IS insurance COMPANY
...
begin
   ...
   dsAccessory.DataSet := Fdm.CdsMtAppGetAccessoryMs;
   ...
   //  set for Approved application
   dsAppr.DataSet := Fdm.CdsMtApprGetList;
...
//   WHSOT5.ListSource := Fdm.GetDSC('WH'); //คลังสินค้า
  ...
  begin
      ...
      begin
         ...
         BtnSelectFinance.Visible := (Fdm.CdsMtAppGetSTPOT5.AsString = 'F');
         ...
         GetImage();
         if Fdm.CdsMtAppGetCTYOT5.AsString = '2' then
           GetAddr(fdm.CdsMtAppGetCNOOT5.AsLargeInt)
         else
           GetAddrAr(fdm.CdsMtAppGetCNOOT5.AsString);
      ...
      begin
         ...
         BtnSelectFinance.Visible := False;
