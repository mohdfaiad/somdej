...
uses
 ...
 type
  TProcOfObject = procedure(DNOPAS : INTEGER) of object;
   ...
   TfrmPopupOrderCol = class(TPopupMainClass)
    UniLabel1: TUniLabel;
    EdtMM: TUniEdit;
    LbCid: TUniLabel;
    EdtYar: TUniEdit;
...
var
  frmPopupOrderCol: TfrmPopupOrderCol;
...
procedure TfrmPopupOrderCol.Init(_DM: TDSAssignWorkDM;_cmp,_brn:string;_yar,_prd:integer;_dcd:string;_date : TDateTime);
begin
  ...
  Fdm.TtPasGetByDat(_cmp,_brn,_yar,_prd,_dcd,_date,'A');// P is from Pre assign
  Fdm.CdsTtPasGetByDate.Open();


end;

procedure TfrmPopupOrderCol.btnPopupConfirmClick(Sender: TObject);
begin
...
procedure TfrmPopupOrderCol.dbgDataListTitleClick(Column: TUniDBGridColumn);
begin
...
procedure TfrmPopupOrderCol.edSearchKeyPress(Sender: TObject;
  var Key: Char);
begin
  if (Key = #13) then
  begin
...
procedure TfrmPopupOrderCol.btnPopupCancelClick(Sender: TObject);
begin
  Close();
