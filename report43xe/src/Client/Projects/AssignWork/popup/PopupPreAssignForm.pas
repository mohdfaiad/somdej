...
uses
 ...
 type
  TProcOfObject = procedure(DNOPAS : INTEGER) of object;
   ...
   TfrmPopupPreAssign = class(TPopupMainClass)
    UniLabel1: TUniLabel;
    EdtMM: TUniEdit;
    LbCid: TUniLabel;
    EdtYar: TUniEdit;
...
var
  frmPopupPreAssign: TfrmPopupPreAssign;
...
procedure TfrmPopupPreAssign.Init(_DM: TDSAssignWorkDM;_cmp,_brn:string;_yar,_prd:integer;_dcd:string;_date : TDateTime);
begin
  ...
  Fdm.TtPasGetByDat(_cmp,_brn,_yar,_prd,'601',_date,'P');// P is from Pre assign
  Fdm.CdsTtPasGetByDate.Open();


end;

procedure TfrmPopupPreAssign.btnPopupConfirmClick(Sender: TObject);
begin
if (FDM.CdsTtPasGetByDate.IsEmpty) then
  ...
  begin
    ...
    if (Assigned(FOnSelectData)) then
      FOnSelectData(FDM.CdsTtPasGetByDateDNOPAS.AsInteger);
...
procedure TfrmPopupPreAssign.dbgDataListTitleClick(Column: TUniDBGridColumn);
begin
...
procedure TfrmPopupPreAssign.edSearchKeyPress(Sender: TObject;
  var Key: Char);
begin
  if (Key = #13) then
  begin
...
procedure TfrmPopupPreAssign.btnPopupCancelClick(Sender: TObject);
begin
  Close();
