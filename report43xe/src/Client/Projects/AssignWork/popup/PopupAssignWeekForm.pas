...
uses
 ...
 type
  TProcOfObject = procedure(DNOPAS : INTEGER) of object;
   ...
   TfrmPopupAssignWeek = class(TPopupMainClass)
    UniLabel1: TUniLabel;
    EdtMM: TUniEdit;
    LbCid: TUniLabel;
    EdtYar: TUniEdit;
...
var
  frmPopupAssignWeek: TfrmPopupAssignWeek;
...
procedure TfrmPopupAssignWeek.Init(_DM: TDSAssignWorkDM;_cmp,_brn:string;_yar,_prd:integer;_dcd:string;_date : TDateTime);
begin
...
procedure TfrmPopupAssignWeek.btnPopupConfirmClick(Sender: TObject);
begin
...
procedure TfrmPopupAssignWeek.dbgDataListTitleClick(Column: TUniDBGridColumn);
begin
...
procedure TfrmPopupAssignWeek.edSearchKeyPress(Sender: TObject;
  var Key: Char);
begin
  if (Key = #13) then
  begin
...
procedure TfrmPopupAssignWeek.btnPopupCancelClick(Sender: TObject);
begin
  Close();
