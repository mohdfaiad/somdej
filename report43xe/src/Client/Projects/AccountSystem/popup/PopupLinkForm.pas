...
uses MainPopupDataForm, uniButton, uniGroupBox, System.Classes, Vcl.Controls,
  ...
  uniLabel, Vcl.StdCtrls, uniCheckBox, uniMemo, uniDBMemo;

type
  TPopupLink = class(TMainPopupData)
    pnBody: TUniPanel;
    gbData: TUniGroupBox;
    pnFooter: TUniPanel;
    BtnPupUpConfirm: TUniButton;
    ...
    CmbLink: TUniComboBox;
    procedure CmbLinkCloseUp(Sender: TObject);
  private
...
var
  PopupLink: TPopupLink;

implementation

{$R *.dfm}

procedure TPopupLink.CmbLinkCloseUp(Sender: TObject);
begin
  if CmbLink.ItemIndex = 0 then
  begin
    LbTaxType12.Caption := 'ภาษีซื้อ';
    Grp3.Visible := False;
    Grp4.Visible := False;
    Grp5.Visible := False;
  ...
  else if CmbLink.ItemIndex = 1 then
  begin
    LbTaxType12.Caption := 'ภาษีขาย';
    Grp3.Visible := False;
    Grp4.Visible := False;
    Grp5.Visible := False;
  ...
  else if CmbLink.ItemIndex = 2 then   // รับทรัพย์สิน
  begin
    Grp12.Visible := False;
    Grp4.Visible := False;
    Grp5.Visible := False;
  ...
  else if CmbLink.ItemIndex = 3 then  // เบิกทรัพย์สิน
  begin
    Grp12.Visible := False;
    Grp3.Visible := False;
    Grp5.Visible := False;
  ...
  else if CmbLink.ItemIndex = 4 then   // ภงด ซื้อ
  begin
    Grp12.Visible := False;
    Grp3.Visible := False ;
    Grp4.Visible := False ;
  ...
  else if CmbLink.ItemIndex = 5 then   // ภงด ขาย
  begin
    Grp12.Visible := False;
    Grp3.Visible := False ;
    Grp4.Visible := False ;
