...
uses
  ...
  Vcl.Imaging.GIFImg, uniImage, uniDBEdit,ServerModuleEx;

type
  TfrmStTargetBySaleManGrid = class(TMainGridForm)
    dsgrid: TDataSource;
    UniLabel1: TUniLabel;
    EdtYar: TUniEdit;
    EdtCom: TUniEdit;
...
var
  frmStTargetBySaleManGrid: TfrmStTargetBySaleManGrid;
...
uses uniGUIForm, XeUniGUIComponents, StTargetBySalemanDataForm;

procedure TfrmStTargetBySaleManGrid.ShowData();
var
  _Form: TUniForm;
begin

  _Form := TUniFormClass(TfrmStTargetBySalemanData).Create(UniApplication);
  TfrmStTargetBySalemanData(_Form).Init(FDM);
...
procedure TfrmStTargetBySaleManGrid.btnAddClick(Sender: TObject);
...
procedure TfrmStTargetBySaleManGrid._Confirm(AResult: Integer);
begin
  case AResult of
    mrYes:
      begin
...
procedure TfrmStTargetBySaleManGrid.btnDeleteClick(Sender: TObject);
begin
...
procedure TfrmStTargetBySaleManGrid.btnEditClick(Sender: TObject);
begin
   if (not Fdm.CdsStBgGet.Active) or
    (Fdm.CdsStBgGet.IsEmpty) then
  begin
...
procedure TfrmStTargetBySaleManGrid.btnSearchClick(Sender: TObject);
var inform: string;
    cirt : String;
    yy : integer;
begin
...
procedure TfrmStTargetBySaleManGrid.BtnViewClick(Sender: TObject);
begin
   if (not Fdm.CdsStBgGet.Active) or
    (Fdm.CdsStBgGet.IsEmpty) then
  begin
...
procedure TfrmStTargetBySaleManGrid.dbgDataListTitleClick(Column: TUniDBGridColumn);
begin
...
procedure TfrmStTargetBySaleManGrid.UniFormCreate(Sender: TObject);
begin
...
//initialization
//  RegisterMainFormClass(TfrmStTargetBySaleManGrid);
